...
| 4 | P1 | Enforce license-to-code gate (pre-commit/pre-push + CI) | The one hard gate is honor-system today; make it mechanical | **DONE** — PR #16 → main `fc6dc0d`, v0.9.0 |
| 5 | P1 | **Requirements Management & Traceability (RMT)** — OPT-IN engine capability: per-project `requirements/registry.yml` + `agent-project/requirements.config.yml` + a config-driven `req-check` CI validator; forward code/test/PR traceability; append-only + safety-area guard; **no background automation**. PROPOSE-ONLY (`massoh-meta-engineer`) → normal gate. First adopter: elard. Full spec → `agent-project/briefs/RMT-requirements-traceability.md`. | Machine-checkable requirements linked to code = Massoh's governance wedge made concrete | TODO |
| 6 | P1 | **Fleet layer (EPIC)** — multi-repo dashboard + cross-repo lessons pool + self-curing engine. Sliced: per-repo board (in flight) → fleet registry+read-only rollup → cross-repo lessons pool → engine self-cure feed. Promotion boundary: only generalizable/de-identified lessons reach the engine; PROPOSE-ONLY + versioned. See [[massoh-fleet-vision]]. Full brief → `agent-project/briefs/fleet-multi-repo-self-curing.md`. | The north-star: each project separate, the master engine improves from all of them | TODO |
